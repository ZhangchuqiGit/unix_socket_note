...
/* ---- accept () 若被系统中断，自己重启 ---- */
int Accept_select(const int &socket_listen_fd);
...
void tcp_service_IO_select()
{
	...
	/* ---- 允许地址的立即重用，端口不占用 ---- */
	int option = 1;
	retval=setsockopt(socket_listen_fd, SOL_SOCKET/* 通用选项 */,
	...
	/* ---- bind () ---- */
	retval=bind(socket_listen_fd, (struct sockaddr *)(&server_address), sizeof(server_address) );
	err_sys(retval, __LINE__, "bind()");

	/* ---- listen () ---- */
	retval=listen(socket_listen_fd, MAX_listen/*最大连接数*/ );
...
/*******************************************************************/
		...
		if (FD_ISSET(socket_listen_fd, &rs)) {
			/* ---- accept () ---- */
			socket_accept_fd = Accept_select(socket_listen_fd);
			/* for select() */
			if (socket_accept_fd > maxfd) maxfd = socket_accept_fd;
			int i;
			for (i = 0; i < FD_SETSIZE; ++i) {
				if (client[i] < 0) {
					client[i] = socket_accept_fd;   /* save descriptor */
					break;
				}
			}
			if (i >= FD_SETSIZE ) {
			...
			FD_SET(socket_accept_fd, &read_set);	// 设置 socket_accept_fd 位
			/* max index in client[] array */
			if (i > client_max_index) client_max_index = i;
			std::cout << "client fd : "
					  << socket_accept_fd << std::endl;
		}
/*******************************************************************/
...
int Accept_select(const int &socket_listen_fd)
{
	...
	socklen_t valsize = sizeof(service_addr);
	while (true) {
		...
		if (socket_accept_fd < 0){
			...
			else err_sys(socket_accept_fd, __LINE__, "accept()");
		}
		else {
			break;
		}
	...
	/* 获取 与某个套接字 关联的 本地/对方 协议地址 */
	getIP_addr_port(socket_accept_fd);
	...
	Get_nameinfo( (struct sockaddr *) (&service_addr), valsize,
				  host, hostlen, serv, servlen);

	return socket_accept_fd; // 新(连接成功)套接字描述符
}
