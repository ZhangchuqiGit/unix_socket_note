...
void tcp_service_OOB_nonblock_select()
{
	...
	ret_value=setsockopt(sock_fd, SOL_SOCKET/*通用选项*/,
						 SO_RCVBUF/* 接收缓冲区 */,
...
/******************* 设置 I/O 非阻塞 **********************/
	_debug_log_info("设置 I/O 非阻塞  listen_fd: %d", sock_fd)
	int flags = Fcntl(sock_fd, F_GETFL, 0); 		// 获取 I/O 状态
	Fcntl(sock_fd, F_SETFL, flags | O_NONBLOCK); 	// 设置 I/O 非阻塞

/********************* I/O口复用 *************************/
	int maxfd = sock_fd;			/* initialize */
	int client_max_index = -1;		/* index into client[] array */
	int client[FD_SETSIZE];
	for (int & i : client) i = -1;	/* -1 indicates available entry */

	fd_set read_set; // 鉴别事件的标志位集合
	FD_ZERO(&read_set); // 初始化集合:所有位均关闭
	FD_SET(sock_fd, &read_set); // 设置 sock_fd 位
	fd_set rs; // 鉴别事件的标志位集合

	fd_set event_set; // 鉴别事件的标志位集合 告知对端发送了一个带外字节
	FD_ZERO(&event_set); // 初始化集合:所有位均关闭
	fd_set es; // 鉴别事件的标志位集合 告知对端发送了一个带外字节
/*******************************************************/
	...
	while (true)
	{
		...
		num_ready=select(maxfd+1, &rs, nullptr,
						 &es, nullptr);
		// 返回就绪描述符的数量
...
/*******************************************************************/
		...
		if (FD_ISSET(sock_fd, &rs)) {
			/* ---- accept () 非阻塞 select() ---- */
			accept_fd = Accept_nunblock_select(sock_fd);
			/* for select() */
			...
			FD_SET(accept_fd, &event_set); 	// 设置 accept_fd 位 一个带外字节
			FD_SET(accept_fd, &read_set);	// 设置 accept_fd 位
...
/*******************************************************************/
		...
		for (int i = 0; i <= client_max_index; ++i) {
			...
			if (FD_ISSET(client[i], &es)) /* socket is readable */
			{
				std::cout << "SIGURG received ";
				retval = Recv(accept_fd, buf, sizeof(buf) - 1, MSG_OOB/*处理带外数据*/);
				buf[retval] = 0;		/* null terminate */
				std::cout << "[ " << retval << "bytes OOB ]: " << buf << std::endl;

				FD_CLR(client[i], &event_set); // 关闭 sockfd_t 位
				flag_event = 1;
			}

			if (FD_ISSET(client[i], &rs)) /* socket is readable */
			{
				retval = Read(client[i], buf, sizeof(buf)-1);
				buf[retval] = 0;		/* null terminate */
...
/* Return the current time and put it in *TIMER if timer is not nullptr */
		...
		对于 b，表示如果字符串长度超过b，那么只取前b个    */
					...
					if (flag_event == 1) {
						flag_event = 0;
						FD_SET(client[i], &event_set);    // 设置 accept_fd 位 一个带外字节
					}
				}
			}
