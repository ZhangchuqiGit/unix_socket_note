...
{
	err_sys(mcast_block_source(sock_fd, src, srclen, grp, grplen),
					   ...
					   const struct sockaddr *grp, const socklen_t &grplen)
{
	...
	return (setsockopt(sock_fd, Family_to_level(grp->sa_family),
					   MCAST_BLOCK_SOURCE/*阻塞多播源*/, &req, sizeof(req)));
#else
	switch (grp->sa_family)
	{
		case AF_INET: {
			...
			return(setsockopt(sock_fd, IPPROTO_IP,
							  IP_BLOCK_SOURCE/*阻塞多播源*/, &mreq, sizeof(mreq)));
		}
...
void Mcast_unblock_source(const int &sock_fd,
						  ...
						  const struct sockaddr *grp, const socklen_t &grplen)
{
	err_sys(mcast_unblock_source(sock_fd, src, srclen, grp, grplen),
			__FILE__, __func__, __LINE__, "%s()", __func__);
}
						 ...
						 const struct sockaddr *grp, const socklen_t &grplen)
{
	...
	memcpy(&req.gsr_source, src, srclen);
	return (setsockopt(sock_fd, Family_to_level(grp->sa_family),
					   MCAST_UNBLOCK_SOURCE/*开通多播源*/, &req, sizeof(req)));
#else
	switch (grp->sa_family)
	{
		case AF_INET: {
			...
			return(setsockopt(sock_fd, IPPROTO_IP,
							  IP_UNBLOCK_SOURCE/*开通多播源*/, &mreq, sizeof(mreq)));
		}
