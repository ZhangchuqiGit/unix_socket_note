...
#ifndef __MULTICAST_H
#define __MULTICAST_H
...
/* 离开多播组 */
void Mcast_leave(const int &sock_fd,
				 const struct sockaddr *grp, const socklen_t &grplen);
/* 离开多播组 */
int mcast_leave(const int &sock_fd,
				const struct sockaddr *grp, const socklen_t &grplen);

/* 离开源特定多播组 */
void Mcast_leave_source_group(const int &sock_fd,
							  const struct sockaddr *src, const socklen_t &srclen,
							  const struct sockaddr *grp, const socklen_t &grplen);
/* 离开源特定多播组 */
int mcast_leave_source_group(const int &sock_fd,
							 const struct sockaddr *src, const socklen_t &srclen,
							 const struct sockaddr *grp, const socklen_t &grplen);
...
/* 阻塞多播源 */
void Mcast_block_source(const int &sock_fd,
						const struct sockaddr *src, const socklen_t &srclen,
						const struct sockaddr *grp, const socklen_t &grplen);
/* 阻塞多播源 */
int mcast_block_source(const int &sock_fd,
					   const struct sockaddr *src, const socklen_t &srclen,
					   const struct sockaddr *grp, const socklen_t &grplen);

/* 开通多播源 */
void Mcast_unblock_source(const int &sock_fd,
						  const struct sockaddr *src, const socklen_t &srclen,
						  const struct sockaddr *grp, const socklen_t &grplen);
/* 开通多播源 */
int mcast_unblock_source(const int &sock_fd,
						 const struct sockaddr *src, const socklen_t &srclen,
						 const struct sockaddr *grp, const socklen_t &grplen);

/*************************************************************************/

/* 指定 超时时间 TTL 或 外出跳限 HOPS */
void Mcast_set_TTL_HOPS(const int &sock_fd, const int &on_off);
/* 指定 超时时间 TTL 或 外出跳限 HOPS */
void Mcast_set_TTL_HOPS(const int &sock_fd, const int &family, const int &on_off);

/* 指定 超时时间 TTL 或 外出跳限 HOPS */
int mcast_set_TTL_HOPS(const int &sock_fd, const int &on_off);
/* 指定 超时时间 TTL 或 外出跳限 HOPS */
int mcast_set_TTL_HOPS(const int &sock_fd, const int &family, const int &on_off);

/* 指定 超时时间 TTL 或 外出跳限 HOPS */
int Mcast_get_TTL_HOPS(const int &sock_fd);
/* 指定 超时时间 TTL 或 外出跳限 HOPS */
int Mcast_get_TTL_HOPS(const int &sock_fd, const int &family);
...
/* 指定外出接口 */
void Mcast_set_IF(const int &sock_fd, const char *ifname,
				  const u_int &ifindex = 0/*内核自动选择*/);
/* 指定外出接口 */
int mcast_set_IF(const int &sock_fd, const char *ifname,
				 const u_int &ifindex = 0/*内核自动选择*/);
