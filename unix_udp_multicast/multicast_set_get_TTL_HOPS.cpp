...
{
	Mcast_set_TTL_HOPS(sock_fd, Sockfd_to_family(sock_fd), on_off);
}

/* 指定 超时时间 TTL 或 外出跳限 HOPS */
void Mcast_set_TTL_HOPS(const int &sock_fd, const int &family, const int &on_off)
{
	err_sys(mcast_set_TTL_HOPS(sock_fd, family, on_off),
...
{
	return mcast_set_TTL_HOPS(sock_fd, Sockfd_to_family(sock_fd), on_off);
}
	...
	switch (family)
	{
		case AF_INET: {
			...
			return (setsockopt(sock_fd, IPPROTO_IP,
						   IP_MULTICAST_TTL/*外出 TTL*/, &ttl, len));
		}
		...
		case AF_INET6: {
			...
			return (setsockopt(sock_fd, IPPROTO_IPV6,
						   IPV6_MULTICAST_HOPS/* 外出跳限 HOPS*/, &hop, len));
		}
...
{
	return Mcast_get_TTL_HOPS(sock_fd, Sockfd_to_family(sock_fd));
}

/* 指定 超时时间 TTL 或 外出跳限 HOPS */
int Mcast_get_TTL_HOPS(const int &sock_fd, const int &family)
{
	int retval = mcast_get_TTL_HOPS(sock_fd, family);
