...
{
	err_sys(mcast_leave(sock_fd, grp, grplen),
				...
				const struct sockaddr *grp, const socklen_t &grplen)
{
	...
	return (setsockopt(sock_fd, Family_to_level(grp->sa_family),
					   MCAST_LEAVE_GROUP/*离开多播组*/, &req, sizeof(req)));
#else
	switch (grp->sa_family)
	{
		case AF_INET: {
			...
			mreq.imr_interface.s_addr = htonl(INADDR_ANY); /*内核自动选择*/
...
#define	IPV6_LEAVE_GROUP	IPV6_DROP_MEMBERSHIP // 旧的定义
#endif
			...
			mreq6.ipv6mr_interface = 0; /*内核自动选择*/

			return(setsockopt(sock_fd, IPPROTO_IPV6, IPV6_LEAVE_GROUP,
							  &mreq6, sizeof(mreq6)));
		}
...
void Mcast_leave_source_group(const int &sock_fd,
							  ...
							  const struct sockaddr *grp, const socklen_t &grplen)
{
	err_sys(mcast_leave_source_group(sock_fd, src, srclen, grp, grplen),
			__FILE__, __func__, __LINE__, "%s()", __func__);
}
...
int mcast_leave_source_group(const int &sock_fd,
	...
	return (setsockopt(sock_fd, Family_to_level(grp->sa_family),
					   MCAST_LEAVE_SOURCE_GROUP/*离开源特定多播组*/,
					   &req, sizeof(req)));
#else
	switch (grp->sa_family) {
